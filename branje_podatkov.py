...
with open("Podatki_z_merjenj.txt", encoding="utf-8") as podatki:
    celotno_besedilo_z_merjenj = podatki.read().replace(
        ", (+)", ",(+)").replace(", (-)", ",(-)").replace(";","-").replace("\n", " ").replace("Z loop 4W", "ZLOOP 4W").replace("Z line 4W", "ZLINE 4W")
    ...
    print(Fore.YELLOW + "Meritve so bile opravljene od:",
          seznam_datumov_po_vrstnem_redu[0], "do:", seznam_datumov_po_vrstnem_redu[-1])
    ...
    def ustvari_seznam_vseh_meritev():
        ...
        for kocka_teksta in loceno_besedilo_na_kocke_teksta:
            ...
            pot_do_druzine_meritev = model.najdi_pot_izven_razreda_Meritev(
                kocka_teksta)
            ...
            for vrsta_meritve in seznam_vrst_meritev:
                if vrsta_meritve in kocka_teksta:
                    slovar_meritev[vrsta_meritve] = kocka_teksta.count(
                        vrsta_meritve)
            ...
            if stevilo_meritev_v_trenutni_kocki == 1:
                ...
                if kocka_teksta.count("p//") == 0:
                    seznam_vseh_meritev.append(
                        [model.Meritev(kocka_teksta.replace("\n", " ").replace("\r\n", " ").strip())])
                    seznam_ustreznih_poti_do_kock.append(
                ...
                seznam_vseh_meritev_brez_poti_na_koncu += [kocka_teksta[i:j] for i, j in zip(
                    seznam_indeksov_posameznih_meritev, seznam_indeksov_posameznih_meritev[1:]+[None])]
                loceno_besedilo_zacasno = []
                for meritev in seznam_vseh_meritev_brez_poti_na_koncu:
                    if meritev.count("p//") == 0:
                        loceno_besedilo_zacasno.append(model.Meritev(
                            meritev.replace("\n", " ").replace("\r\n", " ").strip()))
    ...
    for pripona in vse_pripone_datotek:
        ...
        excel_delovna_datoteka = load_workbook(f"Templati//template_za_{pripona}_meritve.xlsx")
    ...
    for kocka in seznam_vseh_meritev:
            #print([i.besedilo for i in kocka])
            ...
            for i, vrstica in enumerate(vrstice):
                ...
                for element in pot_locena_na_elemente:
                    ...
                    if sekcija != "" and sekcija not in seznam_sekcij:
                        ...
                        top_cell.fill = PatternFill(start_color='F5C77E',
                        end_color='F5C77E',
                        fill_type='solid')
